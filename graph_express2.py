...
import sympy
import numpy as np
from sympy import Sum, IndexedBase, Idx, symbols
...
def main():
    ...
    A, B, C = IndexedBase('A'), IndexedBase('B'), IndexedBase('C')
    ...
    test_suite = [
...

        {
            ...
            'expr_fn': lambda ii, jj: A[ii, jj] + C[ii, jj],
            'dims': (M_val, N_val),
            'data_sources': lambda: {
        ...
        },
        {
            ...
            'expr_fn': lambda ii, jj: Sum(A[ii, k] * B[k, jj], (k, 0, K_val - 1)),
            'dims': (M_val, N_val),
            'data_sources': lambda: {
        ...
        {
            'name': "(A + C) ** 2",
            'expr_fn': lambda ii, jj: (A[ii, jj] + C[ii, jj])**2,
            'dims': (M_val, N_val),
            'data_sources': lambda: {
                'A': np.random.rand(M_val, N_val),
                'C': np.random.rand(M_val, N_val)
